...
void search(int *path,  int length, int destination)
{
    ...
    if(currentNode==destination)
    {
        ...
        minCost=totalCost;      //updating the minimum cost for every possible path
        return;
    }
    for(i=0; i<nodeCount; i++)
    {
    ...
    while(ptr!=NULL)
    {
        ...
        int possibleCost=ptr->cost;
        if(!isInPath(path,length,possibleId))
        {
            path[length]=possibleId;
            totalCost+=possibleCost;                            //Add cost every time it finds a new node
            printf("New node in the path : %d\n",possibleId);
            search(path,length+1,destination);
            totalCost-=possibleCost;                             //Minus the cost every time it backtrack
...
int main()
{
    ...
    printf("\n[ Minimum cost for going from %d to %d : %d ]\n",source,destination,minCost);
}